    ...
    public async Task LoadAsync(DispatcherQueue dispatcherQueue, Server server)
    {
        ...
        try
        {
            foreach (var serverGames in FeaturedGames.GroupBy(g => g.Server)) // While on dev token
            {
                ...
                {
                    var summoner =
                        await client.GetSummonerByNameAsync(participant.Name, _loadCancellationTokenSource.Token);
                    var masteries = await client.GetChampionMasteryAsync((long) participant.ChampionId, summoner.Id,
